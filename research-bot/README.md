...
research-bot/
  ...
  index.json                             ← regenerated by `openslate sign --batch`
```

## Batch signing

Once you have one or more reviewed `positions.json` bundles, sign the whole
tree at once:

```sh
bun run packages/cli/src/index.ts sign --batch research-bot --key me.identity.json
```

This walks `orgs/<slug>/<election>/positions.json`, writes `signed.slate`
next to each, and regenerates `index.json` with one entry per signed bundle
(slug, election, issuer key, attribution summary, position count). Bundles
that already have a `signed.slate` are skipped; pass `--force` to re-sign.

`index.json` is meant for consumers: a single fetch tells them what bundles
exist in this repo, who signed each, and which entity each one reports on.
